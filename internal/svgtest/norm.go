...
var (
	_numberRe     = regexp.MustCompile(`\d+(\.\d+)?`)
	_dataPointsRe = regexp.MustCompile(`data-points="([^"]+)"`)
)
...
func Normalize(s string) string {
	// Strip data-points attributes since they vary across runs.
	s = _dataPointsRe.ReplaceAllString(s, `data-points="NORMALIZED"`)
